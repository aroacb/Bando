...
// --- Llamada a la IA a través de la función serverless (la clave vive en el servidor, no aquí) ---
...
// --- Almacenamiento local del navegador (de momento por dispositivo; Supabase sustituirá esto más adelante) ---
function loadTopicData() {
  const raw = localStorage.getItem("topics-data");
  if (raw) return JSON.parse(raw);
  const initial = { topics: SEED_TOPICS, history: {} };
  localStorage.setItem("topics-data", JSON.stringify(initial));
  return initial;
}
function saveTopicData(data) {
  localStorage.setItem("topics-data", JSON.stringify(data));
}
function loadStreak() {
...
function bumpStreak() {
  ...
  localStorage.setItem("streak-data", JSON.stringify(next));
  return next;
}
  ...
  return (
    <div style={{ background: TOKENS.paperDim, borderRadius: 8, padding: 16, marginTop: 20 }}>
      <p style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, letterSpacing: 0.6, color: TOKENS.ink70, marginBottom: 10 }}>
        TEMAS PROPUESTOS · {category.toUpperCase()} · guardados en este navegador
      </p>
...
Responde SOLO con JSON válido: {"solidez": <0-100>, "evidencia": <0-100>, "falacias": ["..."], "puntos_faltantes": ["..."], "mejor_contraargumento": "..."}. Textos breves (máx 20 palabras).`;
      ...
      const oppText = await askClaude(oppPrompt, 300);
      setOpponent(oppText.trim());
      setRound(2);
    } catch (e) {
...
Responde SOLO con JSON: {"capacidad_respuesta": <0-100>, "comprension_contrario": <0-100>, "comentario": "...", "probabilidad_persuasion": <0-100>}. Comentario máx 25 palabras.`;
  ...
  return (
    <div style={{ minHeight: "100vh", background: TOKENS.ink, padding: "28px 16px", fontFamily: "'Source Sans 3', 'Inter', sans-serif", color: TOKENS.ink, lineHeight: 1.55, WebkitFontSmoothing: "antialiased" }}>
      <style>{`
                  ...
                  <div>
                    ...
                    <div style={{ background: `${accent}1A`, borderRadius: 8, padding: 14, marginTop: 16 }}>
                      ...
                      <p style={{ fontSize: 13, margin: 0 }}>
                        Tu respuesta tendría un {analysis2.probabilidad_persuasion}% de probabilidad estimada de convencer a alguien del bando contrario. Esto vendrá de votos reales de otras personas cuando conectemos Supabase.
                      </p>
                    </div>
                    {streak && (
